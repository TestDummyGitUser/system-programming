...
1. **int8_t (signed byte)** — тип целочисленных чисел, который содержит 8 бит.
...
2. тип `int8_t` хранит 256 значений: 128 отрицательных, 127 положительных, и 1 приходится на ноль.

3. Диапазон асимметричен вследствие того, что ноль тоже занимает одно место.
   ```text
   0000 0000 ->  0
